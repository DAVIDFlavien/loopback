...
AccessContext.prototype.isScopeAllowed = function() {
  ...
  const scopes = this.accessToken.scopes || DEFAULT_SCOPES;

  const requiredScopes = this.getScopes();

  // Scope is allowed when at least one of token's scopes
  // is found in method's scopes.
  return Array.isArray(scopes) && Array.isArray(requiredScopes) &&
    requiredScopes.some(s => scopes.indexOf(s) !== -1);
};
